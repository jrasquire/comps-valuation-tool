...
def fetch_comp(ticker: str) -> CompData:
    ...
    FMP is used because it works reliably from cloud servers (API-key based,
    not IP-based like yfinance) and has a much more generous free daily
    quota (250 requests/day) than Alpha Vantage (25/day), which matters for
    a publicly shared demo app that others may click into and test.
    """
    ...
    try:
        metrics_resp = requests.get(
            f"https://financialmodelingprep.com/api/v3/key-metrics-ttm/{comp.ticker}",
        ...
        profile_resp = requests.get(
            f"https://financialmodelingprep.com/api/v3/profile/{comp.ticker}",
            params={"apikey": api_key},
            timeout=15,
        )
        profile = profile_resp.json()
        comp.name = profile[0].get("companyName", comp.ticker) if profile else comp.ticker
        comp.market_cap = profile[0].get("mktCap") if profile else None
